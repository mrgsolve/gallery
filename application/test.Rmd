...
sp <- chunk_by_row(mat2, nchunk = 4)
sim_this_chunk <- function(chunk, mod, e) {
   mrgsim_ei(mod, e, chunk, output = "df")
}

system.time(out <- future_map(sp, sim_this_chunk, mod = mod, e = sunev()))
future::plan(future::sequential)

system.time(out <- map(sp, sim_this_chunk, mod = mod, e = sunev()))
```
